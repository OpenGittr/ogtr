...
(`server`, `app`), plus namespace, configmap, secrets template and
ingress. Full walkthrough: [docs/DEPLOYMENT.md](../docs/DEPLOYMENT.md).
...
| `server-deployment.yaml` | backend Deployment + Service (probes, metrics annotations, optional GeoIP volume) |
| `app-deployment.yaml` | dashboard SPA Deployment + Service |
| `ingress.yaml` | one Ingress, two hosts (short / app domains) |
...
2. **Images** — no registry is decided yet; build the two images
   (`backend`, `frontend` Dockerfiles,
   `--platform linux/amd64`), push to your registry, and prefix the `image:`
   fields in the two deployments.
