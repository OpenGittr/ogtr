...
func setup(app *gofr.App, o *options) error {
	...
	app.UseMiddleware(auth.Middleware(tokens, o.exemptPaths...),
	...
	// Deliberately absent: the instance admin API (/api/internal/*). It is
	// a separate, cluster-internal service (backend/admin, deployed as
	// ogtr-internal) — the public server never registers any admin route
	// and treats /api/internal/* like any other unknown /api path
	// (ARCHITECTURE.md "Instance admin service").
