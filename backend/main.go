...
func main() {
	...
	if signingKey == "" {
		app.Logger().Fatal("JWT_SIGNING_KEY is not set; refusing to start (session tokens would be unsigned)")
	}
	...
	// AUTH_PROVIDERS decides which sign-in methods exist (ARCHITECTURE.md §5).
	// Unknown values refuse to start; "dev" trusts any submitted identity and
	// exists only so a fresh install can be evaluated without Google setup.
	providerNames, err := auth.ParseProviders(app.Config.Get("AUTH_PROVIDERS"))
	if err != nil {
		app.Logger().Fatalf("refusing to start: %v", err)
	}

	providers := make(map[string]auth.IdentityProvider, len(providerNames))
	...
	if shortDomain == "" {
		app.Logger().Fatal("SHORT_DOMAIN is not set; refusing to start (short URLs cannot be built)")
	}
	...
	app.UseMiddleware(auth.Middleware(tokens), visitor.Middleware(), handlers.CacheControl())

	// Session (ARCHITECTURE.md §4). providers + google + dev + refresh are
	// auth-exempt. The login routes are always registered; the handler
	// answers 404 for providers not in AUTH_PROVIDERS.
